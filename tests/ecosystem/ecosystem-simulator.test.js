import { requireWebGPU, test, verifyWebGPUInitialized } from './webgpu-test-utils.js';
...
test.describe('Ecosystem Simulator - Milestone 1', () => {
	...
	test('should detect WebGPU when available', async ({ page }) => {
		// Check for compatibility notice - this will only run since we require WebGPU
	...
	// Remove test for WebGPU warning dialog since we now require WebGPU
...
test.describe('WebGPU Feature Detection - Milestone 1', () => {
	test('should detect WebGPU availability and features', async ({ page }) => {
		...
		// WebGPU should be supported
		expect(features.webGPU).toBe(true);

		// Web Workers should be supported
