...
export class EcosystemSimulator {
	 ...
	 */
	spawnLifeform(x, y, options = {}) {
		if (!this.state.isInitialized) {
			return null;
		}

		// Convert type strings to species numbers
		let species = 0;
		switch (options.type) {
			case 'predator':
				species = 2;
		...
		const entityOptions = {
			...
			genes: {
				...
				senseRange: options.senseRange || 40,
				metabolism: options.metabolism || 1.0,
				strength: options.strength || 1.0,
			},
			maturity: options.maturity || 0,
		};

		// Create the entity
		const entity = this.features.webWorker
			? this.tileManager.spawnEntity(entityOptions)
			: this.coreSimulation.spawnLifeform(entityOptions);
	...
	async reproduce(parent1Id, parent2Id) {
		...
		// Get parent states
		const parent1 = this.coreSimulation.getLifeformState(parent1Id);
		const parent2 = this.coreSimulation.getLifeformState(parent2Id);
		...
		const child = await this.coreSimulation.reproduce(parent1Id, parent2Id);
		return child;
	}
	...
	getLifeformState(id) {
		...
		return {
			...
			genes: {
				speed: entity.genes.speed,
				senseRange: entity.genes.senseRange,
				metabolism: entity.genes.metabolism,
				strength: entity.genes.size,
			},
	...
	getNeuralNetwork(id) {
		...
		return {
			...
			hiddenLayers: [new Array(network.architecture.hiddenSize).fill(0)],
			outputLayer: ['moveDirection', 'moveSpeed', 'reproduce', 'attack'],
		};
	}
	 ...
	 */
	async step() {
		if (!this.coreSimulation || !this.state.isInitialized) {
			return;
		}
