...
// Export simulator class
export class JessesWorldSimulator {
	/**
	 ...
	 */
	async initialize() {
		console.debug('🌱 Initializing simulator...');
		...
		try {
			...
			// Create main canvas element first - do this regardless of WebGPU support
			...
			try {
				...
				console.warn('WebGPU initialization failed:', gpuError);
			}

			// Fall back to 2D context if WebGPU failed
			if (!this.features.webGPU) {
				console.debug('🌱 Falling back to 2D canvas');
			...
			// Initialize shaders if we have WebGPU
			if (this.features.webGPU) {
				await this.initializeShaders();
			}

			// Enable UI controls - do this even if WebGPU failed
			this.enableControls();
			...
			console.debug('🌱 Simulator initialized successfully');
			return true;
		} catch (error) {
				...
				workerInfo.worker.postMessage(
					{
						...
						tileInfo: {
							...
							width: tile.width,
							height: tile.height,
							neighborIds: tile.neighborTiles.map((/** @type {any} */ n) => {
								return n.id;
							}),
						},
						// Transfer the canvas
	...
	handleWorkerMessage(worker, event) {
		...
		switch (type) {
			case 'tileReady': {
				...
				const tile = this.tiles.find((/** @type {any} */ t) => {
					return t.id === tileId;
				});
				if (tile) {
					tile.ready = true;
			...
			case 'workerStatus': {
				...
				const workerInfo = this.workers.find((/** @type {any} */ w) => {
	...
	enableControls() {
		// Enable spawn button
		const spawnButton = /** @type {HTMLButtonElement} */ (
			document.getElementById('spawn-life')
		);
		if (spawnButton) {
		...
		const toggleButton = /** @type {HTMLButtonElement} */ (
			document.getElementById('toggle-simulation')
		);
		if (toggleButton) {
		...
		const resetButton = /** @type {HTMLButtonElement} */ (
			document.getElementById('reset-preview')
		);
		if (resetButton) {
