/**
 * Chunk Worker - Handles processing for individual chunks in the WebGPU ecosystem simulation
 *
 * This worker is responsible for:
 * 1. Managing entities (lifeforms, resources) within its assigned chunks
 * 2. Computing entity behavior and interactions using WebGPU
 * 3. Rendering its chunk's contents to an OffscreenCanvas
 * 4. Communicating with the main thread and neighboring chunks
 *
 * Each worker may be assigned multiple chunks for efficiency
 */

// Keep track of assigned chunks
...
const renderPipeline = null;
let simulationShaderModule = null;
const renderShaderModule = null;
...
async function initWebGPU() {
	try {
        ...
        fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
          ...
          // Constrain to chunk bounds (bounce off edges)
          if (entity.position.x < 0.0) {
            entity.position.x = 0.0;
            entity.velocity.x = -entity.velocity.x * 0.8;
          }
          if (entity.position.x > f32(params.chunkWidth)) {
            entity.position.x = f32(params.chunkWidth);
            entity.velocity.x = -entity.velocity.x * 0.8;
          }
          ...
          if (entity.position.y > f32(params.chunkHeight)) {
            entity.position.y = f32(params.chunkHeight);
            entity.velocity.y = -entity.velocity.y * 0.8;
          }
...
/**
 * Initialize a chunk with the provided information
 * @param {string} chunkId - The unique ID of this chunk
 * @param {Object} chunkInfo - Information about the chunk
 * @param {OffscreenCanvas} [canvas] - The OffscreenCanvas for this chunk (if supported)
 */
function initializeChunk(chunkId, chunkInfo, canvas) {
	...
	const chunk = {
		...
		lastUpdate: 0,
		ready: false,
		// WebGPU resources for this chunk
		buffers: {
			input: null,
			output: null,
			params: null,
	...
	if (canvas) {
		...
		if (features.webGPU) {
			try {
				...
				chunk.context = canvas.getContext('webgpu');
				const canvasFormat = navigator.gpu.getPreferredCanvasFormat();

				chunk.context.configure({
					device,
					format: canvasFormat,
					alphaMode: 'premultiplied',
				});

				// TODO: Initialize render pipeline for this chunk

				features.offscreenCanvas = true;
			} catch (error) {
				console.warn(
					'WebGPU context on OffscreenCanvas failed, falling back to 2D:',
					error,
				);
	...
	assignedChunks.set(chunkId, chunk);
...
function createChunkBuffers(chunk, maxEntities = 100) {
	...
	chunk.buffers.input = device.createBuffer({
		label: `${chunk.id}-input-buffer`,
		size: maxEntities * ENTITY_SIZE,
		usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST,
	});

	// Create output buffer for entities
	chunk.buffers.output = device.createBuffer({
		label: `${chunk.id}-output-buffer`,
		size: maxEntities * ENTITY_SIZE,
		usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC,
	});

	// Create params uniform buffer
	chunk.buffers.params = device.createBuffer({
		label: `${chunk.id}-params-buffer`,
		size: 16, // deltaTime (f32) + frameCount (u32) + chunkWidth (u32) + chunkHeight (u32)
		usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
	});

	// Create staging buffer for reading results
	chunk.buffers.staging = device.createBuffer({
		label: `${chunk.id}-staging-buffer`,
		size: maxEntities * ENTITY_SIZE,
		usage: GPUBufferUsage.MAP_READ | GPUBufferUsage.COPY_DST,
	});

	// Create bind group for the simulation
	chunk.bindGroups.simulation = device.createBindGroup({
		label: `${chunk.id}-simulation-bindgroup`,
		layout: computePipeline.getBindGroupLayout(0),
		entries: [
			{
				binding: 0,
				resource: { buffer: chunk.buffers.input },
			},
			{
				binding: 1,
				resource: { buffer: chunk.buffers.output },
			},
			{
				binding: 2,
				resource: { buffer: chunk.buffers.params },
			},
		],
	});
}
...
/**
 * Spawn a new entity in the specified chunk
 * @param {string} chunkId - ID of the chunk
 * @param {Object} entityData - Entity data
 */
function spawnEntity(chunkId, entityData) {
	const chunk = assignedChunks.get(chunkId);
	if (!chunk) {
		return;
	}
	...
	if (features.webGPU && chunk.buffers && chunk.entities.length > 100) {
		createChunkBuffers(chunk, Math.ceil(chunk.entities.length * 1.5));
	}
...
/**
 * Update a chunk's simulation with the current entities
 * @param {string} chunkId - ID of the chunk to update
 * @param {number} deltaTime - Time since last update
 */
function updateChunkSimulation(chunkId, deltaTime) {
	const chunk = assignedChunks.get(chunkId);
	...
	if (features.webGPU && device && chunk.buffers && chunk.entities.length > 0) {
		...
		paramsView.setUint32(8, chunk.info.width, true); // chunkWidth
		paramsView.setUint32(12, chunk.info.height, true); // chunkHeight

		device.queue.writeBuffer(chunk.buffers.params, 0, paramsData);

		// Convert entities to buffer data
		const entityData = new ArrayBuffer(32 * chunk.entities.length);
		...
		device.queue.writeBuffer(chunk.buffers.input, 0, entityData);

		// Create a command encoder
		const encoder = device.createCommandEncoder({
			label: `${chunkId}-update-encoder`,
		});

		// Begin a compute pass
		const computePass = encoder.beginComputePass({
			label: `${chunkId}-update-pass`,
		});

		// Set the pipeline and bind groups
		computePass.setPipeline(computePipeline);
		computePass.setBindGroup(0, chunk.bindGroups.simulation);

		// Dispatch the compute work - one workgroup per 64 entities
		const workgroupCount = Math.ceil(chunk.entities.length / 64);
		...
		encoder.copyBufferToBuffer(
			chunk.buffers.output,
			0,
			chunk.buffers.staging,
			0,
			chunk.entities.length * 32,
		);
		...
		readChunkResults(chunk);
	} else {
		// Fallback: Update entities using JavaScript (CPU-based)
		updateEntitiesCPU(chunk, deltaTime);
	}

	// Draw the chunk (either using WebGPU or 2D context)
	drawChunk(chunk);
 ...
 * @param {Object} chunk - The chunk object
 */
async function readChunkResults(chunk) {
	// Map the staging buffer to read the results
	await chunk.buffers.staging.mapAsync(GPUMapMode.READ);

	// Get the mapped buffer range
	const data = new Float32Array(chunk.buffers.staging.getMappedRange());
 ...
 * @param {number} deltaTime - Time since last update
 */
function updateEntitiesCPU(chunk, deltaTime) {
	// Skip if the chunk has no entities
	...
	chunk.entities.forEach((entity) => {
		...
		if (entity.x > chunk.info.width) {
			entity.x = chunk.info.width;
			entity.vx = -entity.vx * 0.8;
		}
		...
		if (entity.y > chunk.info.height) {
			entity.y = chunk.info.height;
			entity.vy = -entity.vy * 0.8;
		}
	...
	chunk.entities = chunk.entities.filter((entity) => {
		return entity.energy > 0;
	});
}

/**
 * Draw a chunk's contents to its canvas
 * @param {Object} chunk - The chunk object
 */
function drawChunk(chunk) {
	// Skip if there's no canvas or context
	if (!chunk.canvas || !chunk.context) {
		return;
	}

	// If we have a WebGPU context, use WebGPU rendering
	if (chunk.context.constructor.name === 'GPUCanvasContext') {
		...
		ctx.fillRect(0, 0, chunk.info.width, chunk.info.height);

		// Draw a border to visualize the chunk (for debugging)
		ctx.strokeStyle = 'rgba(127, 224, 132, 0.5)';
		ctx.lineWidth = 2;
		ctx.strokeRect(0, 0, chunk.info.width, chunk.info.height);
		...
		// Draw chunk ID for debugging
		ctx.fillStyle = 'rgba(255, 255, 255, 0.5)';
		ctx.font = '10px monospace';
		ctx.fillText(chunk.id, 5, 12);
		ctx.fillText(`Entities: ${chunk.entities.length}`, 5, 24);
	}
}

/**
 * Reset all chunks assigned to this worker
 */
function resetAllChunks() {
	for (const [chunkId, chunk] of assignedChunks.entries()) {
		// Clear entities
		chunk.entities = [];
...
function updateLoop() {
	...
	for (const [chunkId, chunk] of assignedChunks.entries()) {
		if (chunk.ready) {
			updateChunkSimulation(chunkId, deltaTime);
		}
	}
	...
	if (frameCount % 60 === 0) {
		postMessage({
			...
			workerId,
			frameCount,
			chunkCount: assignedChunks.size,
			entityCount: Array.from(assignedChunks.values()).reduce((sum, chunk) => {
				return sum + chunk.entities.length;
			}, 0),
		});
	}
...
self.onmessage = async function (event) {
	...
	switch (type) {
		...
		case 'assignChunk':
			...
			initializeChunk(data.chunkId, data.chunkInfo, data.canvas);

			// If we have WebGPU, create buffers for this chunk
			if (features.webGPU && device) {
				const chunk = assignedChunks.get(data.chunkId);
				if (chunk) {
		...
		case 'spawnEntity':
			// Spawn a new entity in the specified chunk
			spawnEntity(data.chunkId, data.entity);
			break;
		...
		case 'updateChunk':
			// Manual update of a single chunk
			if (data.chunkId && assignedChunks.has(data.chunkId)) {
				updateChunkSimulation(data.chunkId, 1 / 60);
			}
			break;
...
console.debug('🌱 Chunk worker initialized');
