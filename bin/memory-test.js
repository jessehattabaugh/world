...
async function runScenario(page, scenario) {
	...
	for (const step of scenario.steps) {
		...
		stepPromises.push(page.waitForTimeout(step.duration));
		...
		// Update max values
		results.summary.maxHeapUsed = Math.max(results.summary.maxHeapUsed, stepEnd.heap.heapUsed);
	}
...
test('Run memory tests', async ({ page }) => {
  ...
  const scenarioPromises = MEMORY_TEST_SCENARIOS.map((scenario) => {
		return runScenario(page, scenario);
  });
